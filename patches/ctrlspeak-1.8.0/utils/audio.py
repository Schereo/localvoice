...
class AudioManager:
    ...
    def __init__(self, transcription_queue, debug_mode=False, app_state=None):
        ...
        # Silero's onset latency (soft first phonemes classified as silence
        # and, previously, dropped outright).
        self._onset_buffer = deque()
        self._onset_samples = 0
        self.ONSET_PREROLL_SAMPLES = int(0.45 * SAMPLE_RATE)
    ...
    def audio_callback(self, indata, frames, time_info, status):
        ...
        """
        if not self.is_collecting:
            # Keep a short pre-roll instead of discarding: whatever was said
            # in the instant before activation joins the next segment.
            self._buffer_onset(indata.copy())
            return
        ...
        if is_speech_chunk:
            if not self.is_potentially_speaking:
                # First speech after silence: prepend the pre-roll so the
                # soft attack VAD missed is part of the segment.
                self._flush_onset_into_buffer()
            ...
            # Silence with no active speech: previously dropped outright,
            # which is exactly where soft word onsets died. Keep it rolling
            # so the next speech onset can reclaim it.
            self._buffer_onset(chunk)
    ...
    def _buffer_onset(self, chunk):
        """Add a chunk to the rolling pre-speech buffer, keeping ~0.45 s."""
        self._onset_buffer.append(chunk)
        self._onset_samples += len(chunk)
        while (
            self._onset_buffer
            and self._onset_samples - len(self._onset_buffer[0]) >= self.ONSET_PREROLL_SAMPLES
        ):
            dropped = self._onset_buffer.popleft()
            self._onset_samples -= len(dropped)

    def _flush_onset_into_buffer(self):
        """Move the pre-speech roll into the segment buffer, oldest first."""
        if self._onset_buffer:
            self.audio_buffer.extend(self._onset_buffer)
